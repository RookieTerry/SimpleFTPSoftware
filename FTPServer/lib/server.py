...
def client_auth(client_socket,args):
    ...
    if client_user.check_user():
        ...
        password,totalspace,userspace = msg.split()
...
def mkdir(client_socket,client_user,ret_data):
    """
    创建目录
    :param client_socket: 客户端socket对象
    :param client_user: 客户端用户对象
    :param ret_data: 接收客户命令消息体  例如：mkdir test
    :return:
    """
    mkdir_folder = ret_data.split(" ")[1]
    if mkdir_folder:
        try:
            folder_path = os.path.join(client_user.homepath, mkdir_folder)
