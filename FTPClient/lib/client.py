...
class client(object):
    ...
    def mkdir(self,command):
        """
        创建目录
        :param command: 发送命令消息格式；mkdir test或mkdir /test/zty
        :return:
        """
        #发送命令消息给服务端
        self.client.sendall(bytes(command,encoding='utf-8'))
        #接收服务端发来的回应消息
        mkdir_msg = str(self.client.recv(1024), encoding='utf-8')
        mkdir_msg = int(mkdir_msg)
        if mkdir_msg == code.FILE_MK_SUCC:
            common.message(">>>>>>>创建目录成功","INFO")
        elif mkdir_msg == code.FILE_MK_FAIL:
